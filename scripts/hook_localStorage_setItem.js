...
if (!window.__ADB_OBSERVER__?.isInstalled?.("hook_localStorage_setItem")) {
// ==UserScript==
...
(function () {
    ...
    function initHook() {
        if (adbInitialized || window.__ADB_OBSERVER__?.isInstalled?.("hook_localStorage_setItem")) return;
        ...
        localStorage.setItem = function () {
            if (flag === "0") {
                console.log(adbLogText("log_local_storage_set", "Set localStorage entry:\n{0}:{1}", [`${arguments[0]}`, `${arguments[1]}`]));
                ...
                if (arguments[0] && param.some(item => arguments[0].includes(item))) {
                    console.log(adbLogText("log_local_storage_set_matched", "Matched localStorage write\nKey ---> {0} Value ---> {1}", [`${arguments[0]}`, `${arguments[1]}`]));
                    window.__ADB_OBSERVER__?.emit(SCRIPT_ID, "call", { arguments });
                    if (is_debugger === "1") {
                        debugger;
                    }
