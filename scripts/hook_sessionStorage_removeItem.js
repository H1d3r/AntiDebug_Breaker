...
if (!window.__ADB_OBSERVER__?.isInstalled?.("hook_sessionStorage_removeItem")) {
// ==UserScript==
...
(function () {
    ...
    function initHook() {
        if (adbInitialized || window.__ADB_OBSERVER__?.isInstalled?.("hook_sessionStorage_removeItem")) return;
        ...
        sessionStorage.removeItem = function () {
            if (flag === "0") {
                console.log(adbLogText("log_session_storage_remove", "Removed sessionStorage entry\nKey ---> ") + arguments[0]);
                ...
                if (arguments[0] && param.some(item => arguments[0].includes(item))) {
                    console.log(adbLogText("log_session_storage_remove_matched", "Matched sessionStorage removal ---> {0}", [`${arguments[0]}`]));
                    window.__ADB_OBSERVER__?.emit(SCRIPT_ID, "call", { arguments });
                    if (is_debugger === "1") {
                        debugger;
                    }
